    ...
    return (
        <View style={{ flexDirection: 'row', height: 130, borderRadius: 10, padding: 10, marginBottom: 10, backgroundColor: colors.tertiary, borderWidth: 0.1 }}>
            <TouchableOpacity onPress={() => navigation.navigate('DetailProductScreen', { product: product })} style={{ backgroundColor: 'white', width: 130, height: 110, borderRadius: 8, borderWidth: 0.1, justifyContent: 'center', alignItems: 'center' }}>
                <Image source={{ uri: product?.main_image }} style={{ height: 100, width: 120, borderRadius: 8 }} resizeMode='cover' />
            </TouchableOpacity>
            <View style={{ width: '70%', marginLeft: 8 }}>
                ...
                {product?.selectedVariants && Object.keys(product.selectedVariants).length > 0 && (
                    <View style={{ marginTop: 5, marginBottom: 5 }}>
                        {Object.entries(product.selectedVariants).map(([groupName, variant], index) => (
                            <Text key={index} style={{ fontSize: 12, color: '#555555' }} numberOfLines={2}>
                                {groupName} : <Text style={{ fontWeight: '600' }}>{variant.name}</Text>
                            </Text>
                        ))}
                    </View>
                )}
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '80%', marginTop: 10, }}>
                    <View style={{ flexDirection: 'row', width: '60%', justifyContent: 'space-around' }}>
                        <TouchableOpacity onPress={() => handleDecrement(product.uniqueKey || product.token)} style={{}}>
                            {loadingButton ? (
                        ...
                        <TouchableOpacity onPress={() => handleIncrement(product.uniqueKey || product.token)} style={{}}>
                            {loadingMinus ? (
                                <ActivityIndicator size={22} color="black" />
                            ) : (
                    ...
                    <TouchableOpacity onPress={() => handleRemoveFromCart(product.uniqueKey || product.token)} style={{ backgroundColor: 'red', borderRadius: 15, width: 30, height: 30, justifyContent: 'center', alignItems: 'center' }}>
                        {loading ? (
